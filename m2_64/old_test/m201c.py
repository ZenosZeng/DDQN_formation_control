...
def car2state(Leader,Follower):
    theta_f = theta_cali(Follower.theta)
    theta_l = theta_cali(Leader.theta)
    ex =  Leader.x - Follower.x
    ey =  Leader.y - Follower.y
    e_theta = theta_l-theta_f
    e1 = ex*cos(theta_f)+ey*sin(theta_f)
    e2 = -ex*sin(theta_f)+ey*cos(theta_f)
    vf = Follower.v
...
for t in range(time_total):
    ...
    for i in range(1,car_total):
        state = car2state(car[com[i]],car[i])
...
plt.ylabel('y/m')
plt.legend()
plt.savefig('./record/m201c_trajectory.png')
...
for i in range(1,car_total):
    ex_list[i] = [ coor_list[0][0][j] - coor_list[i][0][j] for j in range(time_total) ]

plt.ylim(-3,3) # x error
...
for i in range(1,car_total):
    ey_list[i] = [ coor_list[0][1][j] - coor_list[i][1][j] for j in range(time_total) ]
...
plt.title('y_error')
# plt.legend()
plt.savefig('./record/m201c_XYerror.png')
# finish
print('Error Ploted.')
...
ani.save('./record/m201c_animation.gif') 
