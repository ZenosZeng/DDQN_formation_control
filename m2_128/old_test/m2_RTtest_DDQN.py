...
def car2state(Leader,Follower,nof,com,formation):
    ...
    xdd = xd*cos(Leader.theta)-yd*sin(Leader.theta)
    ydd = yd*cos(Leader.theta)+xd*sin(Leader.theta)

    theta_f = theta_cali(Follower.theta)
    theta_l = theta_cali(Leader.theta)
    ex =  Leader.x - Follower.x - xdd
    ey =  Leader.y - Follower.y - ydd
...
class ModelTest():
    ...
    # run the Env
        for t in range(self.time_total):
            # update leader
            self.car[0].run(self.leadercontrol[t][0],self.leadercontrol[t][1])
    ...
    def Plot(self):
        ...
        t=0
        for i in range(self.car_total):
            line[i]=ax.plot(self.coor_list[i][0][:t],self.coor_list[i][1][:t],color=color_list[i],label=label_list[i])
        ...
        ani=FuncAnimation(fig,update,interval=20,frames=self.time_total-1) 
...
if( RT1A == 1 ):
    ...
    for t in range(time):
        if 150<t<170:
            ...
            pass
    testscene = ModelTest(5,time,'m2_RT1A',startpos,leaderU,com,formation)
    testscene.getTra()
    testscene.Plot()
...
if( RT1B == 1 ):
    ...
    for t in range(time):
        if 150<t<170:
            ...
            pass
    testscene = ModelTest(5,time,'m2_RT1B',startpos,leaderU,com,formation)
    testscene.getTra()
    testscene.Plot()
