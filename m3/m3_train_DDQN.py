...
if(New_model==True):
    ...
    t_model = torch.load('m2_DDQN_net')
    episode_rewards = [] #  reward list for each episode

    # t_model = torch.load('m3_DDQN_tnet')
    # episode_rewards = list( np.loadtxt('m3_Train_reward.txt') )
    # eps_start=eps_final
...
def checkpoint(model,model2,reward_list):
    # save net
    torch.save(model,'./checkpoint/m3_DDQN_net_'+str(len(reward_list)))
    torch.save(model2,'./checkpoint/m3_DDQN_tnet_'+str(len(reward_list)))
    # plot reward
    plt.figure()
    ...
    plt.xlabel('epochs')
    plt.ylabel('reward')
    plt.savefig('./checkpoint/m3_Train_reward_'+str(len(reward_list))+'_epoch'+'.png')
    plt.close()
    # save reward
    np.savetxt('./checkpoint/m3_Train_reward_'+str(len(reward_list))+'_epoch'+'.txt',np.array(reward_list))
...
#-------------------------------------------------------------------------------------
# save NETWORK
torch.save(model,'./record/m3_DDQN_net_finish')
torch.save(t_model,'./record/m3_DDQN_tnet_finish')
np.savetxt('./record/checkpoint/m3_Train_reward.txt',np.array(episode_rewards))
...
plt.xlabel('epochs')
plt.ylabel('reward')
plt.savefig('./record/m3_Train_reward.png')
...
plt.xlabel('epochs')
plt.ylabel('reward')
plt.savefig('./record/m3_Val_reward.png')
