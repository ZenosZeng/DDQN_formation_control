...
for t in range(time_total):
    ...
    # update follower 1
    ex01 = car[0].x-car[1].x
    ey01 = car[0].y-car[1].y
    ei_list[0][0] += ex01/10
    ei_list[0][1] += ey01/10
    ...
    # update follower 2
    ex02 = car[0].x-car[2].x
    ey02 = car[0].y-car[2].y
    ei_list[1][0] += ex02/10
    ei_list[1][1] += ey02/10
...
plt.ylabel('y/m')
plt.legend()
plt.savefig('./record/m1_eval_2a_trajectory.png')
...
plt.title('y_error')
# plt.legend()
plt.savefig('./record/m1_eval_2a_xy_error.png')
...
ani.save('./record/m1_eval_2a_animation.gif') #显示动画
