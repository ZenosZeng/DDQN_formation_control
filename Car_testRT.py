    ...
    # run the Env
        for t in range(self.time_total):
            ...
            for i in range(1,self.car_total):
                leader = self.car[self.com[t][i]]
                follower = self.car[i]
                
                ex,ey,xdd,ydd = car2state(self.car[self.com[t][i]],self.car[i],i,self.com[t],self.formation)
                etheta = -atan( leader.w*xdd/(leader.v-leader.w*ydd) )

                self.car[i].reset(self.car[i].x+ex,self.car[i].y+ey,\
                                  self.car[self.com[t][i]].theta+etheta,self.car[self.com[t][i]].v,self.car[self.com[t][i]].w)
...
if( RTtest == 1 ):
    ...
    for t in range(time):
        if 150<t<170:
            ...
            pass
    testscene = ModelTest(5,time,'RTtest',startpos,leaderU,com,formation)
    testscene.getTra()
    testscene.Plot()
