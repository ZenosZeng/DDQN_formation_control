...
def car2state(Leader,Follower,nof):
    ...
    xdl = xd*cos(theta_l)-yd*sin(theta_l)
    ydl = xd*sin(theta_l)+yd*cos(theta_l)

    ex =  Leader.x - Follower.x - xdl
    ey =  Leader.y - Follower.y - ydl
...
plt.ylabel('y/m')
plt.legend()
plt.savefig('./record/m203a_trajectory.png')
...
plt.title('y_error')
# plt.legend()
plt.savefig('./record/m203a_XYerror.png')
# finish
print('Error Ploted.')
...
ani.save('./record/m203a_animation.gif') 
